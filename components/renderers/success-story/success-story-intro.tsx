...
export default function SuccessStoryIntro({ headline, summaryBullets, keyResults }: SuccessStoryIntroProps) {
  return (
    <section className="my-12">
      <div className="p-6 bg-green-50 rounded-lg border border-green-100 mb-8">
        <h2 className="text-2xl font-bold text-gray-900 mb-4">{headline}</h2>
        <ul className="space-y-2">
          {summaryBullets.map((bullet, index) => (
            <li key={index} className="flex items-start">
              <span className="inline-flex items-center justify-center w-6 h-6 rounded-full bg-green-600 text-white mr-3 flex-shrink-0">
          ...
          <div className="grid md:grid-cols-3 gap-4">
            {keyResults.map((result, index) => (
              <div key={index} className="bg-white border border-gray-200 rounded-lg p-4 shadow-sm">
                <p className="text-3xl font-bold text-green-600 mb-2">{result.value}</p>
